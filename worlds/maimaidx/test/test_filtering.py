...
class DifficultyRanges(RotNTestBase):
    def test_all_difficulty_ranges(self) -> None:
        ...
        dlc_set = {x for x in rotn_world.chuni_collection.DLC}
        intensity_min = rotn_world.options.min_level
        intensity_max = rotn_world.options.max_level
        difficulty = rotn_world.options.difficulty_option

        def test_range(lower, upper, diff):

            songs = rotn_world.chuni_collection.getSongsWithSettings(dlc_set, False, lower, upper)
            for songKey in songs:
                song = rotn_world.chuni_collection.song_items[songKey]
                if song.diff_easy is -1 and "Easy" in diff and upper <= song.diff_easy <= upper:
                    continue

                if song.diff_advanced is -1 and "Medium" in diff and upper <= song.diff_advanced <= upper:
                    continue

                if song.diff_expert is -1 and "Hard" in diff and upper <= song.diff_expert <= upper:
                    continue

                if song.diff_master is -1 and "Impossible" in diff and upper <= song.diff_master <= upper:
                    continue
