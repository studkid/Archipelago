...
from .WaccaCollections import WaccaCollections
from .items import WaccaSongItem, WaccaFixedItem
from .locations import WaccaLocation

class WaccaWeb(WebWorld):
    theme = "stone"
    tutorials = [Tutorial(
        "Multiworld Setup Guide",
        "A guide to setting up Wacca ap",
        "English",
        "setup_en.md",
        "setup/en",
        ["studkid"]
...
class WaccaWorld(World):
    ...
    filler_item_names = list(wacca_collection.filler_items.keys())
    filler_item_weights = list(wacca_collection.filler_weights.values())
    ...
    item_name_groups = wacca_collection.getItemNameGroups()
    ...
    def generate_early(self):
        ...
        if re_gen_passthrough and self.game in re_gen_passthrough:
            ...
            if "finalSongIDs" in slot_data:
                ...
                self.included_songs = [key for key, song in self.wacca_collection.song_items.items() if song.song_id in final]
        ...
        while True:
            available_song_keys = self.wacca_collection.getSongsWithSettings(self.options, min_diff, max_diff)
    ...
    def create_item(self, name: str) -> Item:
        if name == self.wacca_collection.SHEET_NAME:
            return WaccaFixedItem(name, ItemClassification.progression_skip_balancing,
                                 self.wacca_collection.SHEET_CODE, self.player)
        
        filler = self.wacca_collection.filler_items.get(name)
        if filler:
            return WaccaFixedItem(name, ItemClassification.filler, filler, self.player)
    ...
    def create_items(self) -> None:
        ...
        for _ in range(0, item_count):
            self.multiworld.itempool.append(self.create_item(self.wacca_collection.SHEET_NAME))
    ...
    def create_regions(self) -> None:
        ...
        for name in all_selected_locations:
            for j in range(2):
                loc = WaccaLocation(self.player, f"{name}-{j}", self.wacca_collection.song_locations[f"{name}-{j}"], menu_region)
                loc.access_rule = lambda state, item=name: state.has(item, self.player)
                menu_region.locations.append(loc)

    def set_rules(self) -> None:
        self.multiworld.completion_condition[self.player] = lambda state: \
            state.has(self.wacca_collection.SHEET_NAME, self.player, self.get_sheet_win_count())
    ...
    def fill_slot_data(self):
        return {
            ...
            "sheetName": self.wacca_collection.SHEET_NAME,
