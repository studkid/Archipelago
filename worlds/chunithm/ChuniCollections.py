...
class ChuniCollections:
    SHEET_NAME: str = "Map Progress"
    SHEET_CODE: int = 1
    logger = logging.getLogger("Chunithm")

    regions: List[str] = ["jp", "intl", "omni"]
    ...
    def getSongsWithSettings(self, options, diff_lower: int, diff_higher:int) -> List[str]:
        ...
        for key, data in self.song_items.items():
            ...
            if data.difficulties[4] != None and "Ultima" in options.difficulty_option and diff_lower <= data.difficulties[4] * 10 <= diff_higher:
                filtered_list.append(key)
                continue
